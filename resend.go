...
func NewFSResend(dir string, p AsyncProducer, opts ...FSResendOption) *FSResend {
	f := &FSResend{
		dir:           dir,
		logger:        zap.NewNop(),
...
func (f *FSResend) Resend() error {
	...
	for _, d := range dd {
		...
		case <-t.C:
			...
			err = f.retryP.SendMessage(&sarama.ProducerMessage{
				Topic:    msg.Topic,
			...
			if err != nil {
				f.logger.Error("failed to resend message", zap.String("path", path), zap.Error(err))
			}
